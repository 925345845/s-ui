export default {
  ...
  relay: {
    ...
    creatingStatus: "{stage} · {elapsed}s elapsed",
    creatingHint: "Preparing local addresses and relay configuration. Internet checks follow the selected option. Do not submit again.",
    creationStages: {"preparing": "Preparing", "ipv4": "Checking IPv4 upstreams", "addresses": "Adding IPv6 addresses", "dad": "Waiting for addresses", "ipv6": "Checking IPv6 egress", "saving": "Saving nodes", "config": "Building proxy configuration", "starting": "Starting proxy service", "committing": "Committing configuration", "rollback": "Cleaning up failed batch", "done": "Created", "failed": "Failed", "settling": "Allowing IPv6 network to settle"},
    partialResult: "Requested {requested}: created {created}, not created {skipped}.",
    unmatchedIPv4: "Original IPv4 rows not created",
    matchedRows: "IPv4 row {ipv4} ↔ IPv6 candidate {ipv6}",
    partialHint: "IPv6 candidates and IPv4 upstreams are checked separately, then matched in input order. Failure row numbers refer to the candidate or upstream list according to the stage.",
    copySkipped: "Copy skipped rows",
    skippedRow: "Original row {row}",
    verifyEgress: "Check Internet connectivity before creation",
    verifyEgressHint: "For paired/dualstack mode, keep filling automatically and allow replacement IPv6 additions. When disabled, create once without Internet checks.",
    egressUnchecked: "Internet connectivity was not checked. Test on your actual websites.",
    fillProgress: "Matched {matched} / {total} IPv4 upstreams",
    fillRunning: "Filling in the background. Successful nodes are saved in one pool and ready to use.",
    fillDone: "All upstreams have been matched.",
    fillStopped: "Filling stopped. Created nodes are retained.",
    fillStop: "Stop filling and keep created nodes",
    fillError: "Filling stopped. Created nodes are retained. Check server configuration or service status before submitting remaining upstreams.",
    fillHint: "Generate and test IPv6 replacements until every IPv4 is matched or you stop. All successful rounds are appended to one relay pool. Filling continues when the page closes.",
    pairedPlan: "Keep filling usable IPv6 for {count} IPv4 upstreams. Replace failed candidates and save all successful rounds in one pool immediately.",
    ipv6EgressUnavailable: "Проверка TCP-доступа IPv6 {address} не прошла после повторных попыток. Создание отменено. Ошибки целевых адресов:",
