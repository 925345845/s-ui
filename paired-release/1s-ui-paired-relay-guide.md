...
4. 默认开启公网检测。提交 IPv4 上游后自动生成、检测并匹配 IPv6，失败候选清理并换新地址继续测试，直到全部匹配或手动停止。每轮最多 16 条，间隔 5 秒，同一次任务的成功结果立即追加到同一个中转批次，可统一复制或导出；页面只显示已匹配数量，不列失败记录。关闭页面不会停止任务；面板/VPS 重启会终止未完成任务，但已保存节点保留。重启后仅重新提交未匹配上游。IPv4 本身不可用或 VPS 没有更多可用 IPv6 时将继续重试，可手动停止。
