...
4. 按上游数量准备 IPv6 候选，也可填写现成地址；默认自动检测 IPv6 和 IPv4 SOCKS5，通过后分别按输入顺序匹配。失败候选不占用 IPv4，上游或地址不足时只创建实际配对数量，结果列出未创建的 IPv4 原行号。IPv6 检测总时限随数量延长，500 条预算约 20 分钟，成功提前结束。
