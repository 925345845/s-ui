...
本补丁版本为 **v1.5.20-ipv6fix.4**，直接基于 **v1.5.20**（`f4a0d051473abe38404e51d14df41ac37dfed150`）构建，不包含 v1.5.21 及以后版本的功能、依赖升级或数据库变更。

本次优化：

- 新增地址按 `/128` 绑定，生成与轮换仍使用所选 `/64` 等分配前缀；避免为每个地址添加整段直连路由。已有地址保持原绑定。
- 自动识别基准地址时尊重界面填写的前缀，避免首个已绑定 `/128` 被当成整个地址池范围。
- 回滚及删除读取地址实际绑定前缀，兼容原有 `/64` 和新增 `/128`；恢复地址同样使用 `/128`。
- 新增 Linux 隔离网络测试，实际验证批量添加、就绪、轮换范围、回滚、恢复及旧地址清理。

用户实测已确认随机地址以 `/128` 绑定后可连通两个 TCP 检测目标；此前失败仍可能包含网络波动，本补丁不跳过公网检测。
...
```bash
curl -fsSL https://raw.githubusercontent.com/925345845/s-ui/v1.5.20-ipv6fix.4/paired-release/install-s-ui-paired-online.sh | bash
```
