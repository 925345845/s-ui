...
本补丁版本为 **v1.5.20-ipv6fix.8**，直接基于 **v1.5.20**（`f4a0d051473abe38404e51d14df41ac37dfed150`）构建，不包含 v1.5.21 及以后版本的功能、依赖升级或数据库变更。
...
- 每轮最多处理 16 条，未匹配上游轮流重试，轮次间隔 5 秒。每轮成功结果立即保存为一个中转批次（名称后缀 -001、-002 等），可立即使用。
- 页面仅显示已匹配/总数量、任务状态和停止按钮，不列失败记录。关闭页面后后台继续，再次打开可查看进度；面板服务或 VPS 重启会终止尚未完成的任务，但已经保存的节点保留。重启后如需补齐，请只重新提交未匹配的 IPv4。
- 只清理本任务新加且未保存的 IPv6，不删除外部脚本预先添加的地址。手动停止会取消未提交的当前轮，已提交批次保留；数据库、代理配置或地址清理出错会停止任务，避免反复创建错误配置。
...
```bash
curl -fsSL https://raw.githubusercontent.com/925345845/s-ui/v1.5.20-ipv6fix.8/paired-release/install-s-ui-paired-online.sh | bash
```
