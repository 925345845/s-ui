...
本补丁版本为 **v1.5.20-ipv6fix.2**，直接基于 **v1.5.20**（`f4a0d051473abe38404e51d14df41ac37dfed150`）构建，不包含 v1.5.21 及以后版本的功能、依赖升级或数据库变更。
...
- 新建表单默认关闭“Apple ID 专用 IPv4”，普通 IPv4/IPv6 配对按目标地址族分流；仍可手动开启原来的 Apple ID 专用模式，已有批次不变。
...
```bash
curl -fsSL https://raw.githubusercontent.com/925345845/s-ui/v1.5.20-ipv6fix.2/paired-release/install-s-ui-paired-online.sh | bash
```
